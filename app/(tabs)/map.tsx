...
// ✅ CORRIGÉ : Utilise la même source available_orders_filtered que available-orders.tsx
// ✅ CORRIGÉ : Ajoute la déduplication pour cohérence parfaite
// ✅ CORRIGÉ : Exclut les propres demandes de l'utilisateur
// 🔧 CORRIGÉ : Navigation vers /orders/[id] avec validation rigoureuse des IDs
// 🔧 CORRIGÉ : Système de catégories basé sur services.categorie (vraie source)
// 💰 CORRIGÉ : Logique simple fourmiz_amount (accepte les valeurs 0€)
// 🔧 CORRIGÉ : Colonne "urgent" au lieu de "urgency" 
// ✅ NOUVEAU : Suppression complète des références à la table categories
// ✅ NOUVEAU : Utilisation de service_category_text au lieu de service_category_id
...
export default function MapScreen() {
  ...
  const debugCategoriesRelations = async () => {
    ...
    try {
      ...
      console.log('🛠️ Catégories services.categorie:', servicesCategoriesCount.size);
  ...
  const refreshUserLocationAndData = async () => {
    ...
    try {
      ...
      if (status !== 'granted') {
        console.log('⚠️ Permissions GPS non accordées, skip refresh position');
        return;
      }
  ...
  const loadNearbyFourmiz = async (region: MapRegion) => {
    try {
      ...
      if (!data || data.length === 0) {
        console.log('⚠️ Aucun fourmiz trouvé avec les critères actuels');
        setFourmizList([]);
        return;
      }
  ...
  const handleContactAction = (id: string) => {
    ...
    if (userMode === UserMode.CLIENT) {
      ...
      console.log('👤 Navigation CLIENT vers services avec fourmizId:', id);
      router.push(`/services?fourmizId=${id}`);
    } else {
